	...
	class RdataTB {
		constructor(
			...
			if (!Options.ShowSelect && Options.hasOwnProperty("ShowSelect")) {
				(_a = document.getElementById("my-select")) === null ||
				_a === void 0
					? void 0
					: _a.remove();
			}
			this.ShowHighlight =
				Options === null || Options === void 0
					? void 0
					: Options.ShowHighlight;
			if (Options.fixedTable && Options.hasOwnProperty("fixedTable")) {
				(_b = this.TableElement) === null || _b === void 0
					? void 0
					: _b.classList.add("table_layout_fixed");
			} else {
				(_c = this.TableElement) === null || _c === void 0
					? void 0
					: _c.classList.remove("table_layout_fixed");
			}
			if (!Options.ShowSearch && Options.hasOwnProperty("ShowSearch")) {
				(_d = document.getElementById("SearchControl")) === null ||
				_d === void 0
					? void 0
					: _d.remove();
			}
		...
		detectTyped() {
			var _a;
			const getHead =
				(_a = this.TableElement) === null || _a === void 0
					? void 0
					: _a.getElementsByTagName("th");
		...
		PaginateUpdate() {
            ...
            <a style="">Page ${this.i + 1} to ${this.Divide().length} of ${
					this.DataTable === undefined ? 0 : this.DataTable.length
				} Entries</a>`;
			}
		}
		search() {
			...
			(_a = document.getElementById("SearchControl")) === null ||
			_a === void 0
				? void 0
		...
		ConvertToJson() {
			...
			for (let v = 0; v < getHead.length; v++) {
				(_b = this.HeaderDataTable) === null || _b === void 0
					? void 0
					: _b.push(getHead[v].textContent);
			}
			//get row data
			const getbody =
				(_c = this.TableElement) === null || _c === void 0
					? void 0
					: _c.getElementsByTagName("tbody");
			for (
				let row = 0;
				row < (getbody[0] === undefined ? 0 : getbody[0].rows.length);
				row++
			) {
				const cellsD = [];
				for (
		...
		RenderToHTML(SlecTloaf = null) {
			...
			if (SlecTloaf === null) {
				for (let ___row = 0; ___row < ifUndefinded; ___row++) {
					...
					for (
						...
						ToCell += `<td class="${
							this.HeaderDataTable[___cell]
						}__row">${
							this.DataToRender[___row][
								this.HeaderDataTable[___cell]
							]
						}</td>\n`;
					}
				...
				for (let ___row = 0; ___row < SlecTloaf.length; ___row++) {
					...
					for (
						...
						ToCell += `<td class="${
							...
							SlecTloaf[___row][this.HeaderDataTable[___cell]]
						}</td>\n`;
					}
					row += `<tr>${ToCell}</tr>\n`;
				}
			...
			for (let n = 0; n < this.HeaderDataTable.length; n++) {
				const cv = document.getElementById(
					`${this.HeaderDataTable[n]}_header`
				);
		...
		sort(column) {
			...
			const isDate =
				(listDated === null || listDated === void 0
					? void 0
					: listDated.HeaderIndex) === IndexHead;
				...
				for (let row = 0; row < getbody[0].rows.length; row++) {
					for (
						...
						if (index >= 0) {
							...
							getbody[0].rows[row].cells[cellsIndex].innerHTML =
								innerHTML;
							getbody[0].rows[row].cells[
								cellsIndex
							].classList.add(
								`${this.HeaderDataTable[cellsIndex].replace(
									/\s/g,
									"_"
								)}__row`
							);
