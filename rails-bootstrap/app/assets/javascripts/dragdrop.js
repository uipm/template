...
var sortable = (function () {
	...
	var Store = /** @class */ (function () {
		...
		Store.prototype.setConfig = function (key, value) {
			if (!this._config.has(key)) {
				throw new Error(
					"Trying to set invalid configuration item: " + key
				);
			}
			// set config
	...
	var makePlaceholder = function (
		...
		if (typeof placeholderClass === "string") {
			(_a = placeholder.classList).add.apply(
				_a,
				placeholderClass.split(" ")
			);
		}
		return placeholder;
	};
			...
			items
				...
				.filter(function (item) {
					return (
						...
						(item.shadowRoot &&
							item.shadowRoot.querySelector(selector) instanceof
								HTMLElement)
					);
				})
	...
	var enableHoverClass = function (sortableContainer, enable) {
		if (
			typeof store(sortableContainer).getConfig("hoverClass") === "string"
		) {
						...
						if (event.buttons === 0) {
							filter(
								...
								if (
									...
									(_a = item.classList).add.apply(
										_a,
										hoverClasses_1
									);
								} else {
									(_b = item.classList).remove.apply(
										_b,
										hoverClasses_1
									);
								}
	...
	function sortable(sortableElements, options) {
		...
		sortableElements.forEach(function (sortableElement) {
			...
			if (
				...
				var tempContainer = document.createElement(
					sortableElement.tagName
				);
          ...
          */
			addEventListener(sortableElement, "dragenter", function (e) {
				...
				if (
					sortableContainer &&
					sortableContainer !== previousContainer
				) {
			...
			addEventListener(sortableElement, "drop", function (e) {
				...
				var destinationElementIndex = getIndex(
					dragging,
					Array.from(dragging.parentElement.children).filter(
						function (item) {
							return item !== placeholder;
						}
					)
			...
			var debouncedDragOverEnter = debounce(function (
				...
				if (options.forcePlaceholderSize) {
					store(sortableElement).placeholder.style.height =
						draggingHeight + "px";
					store(sortableElement).placeholder.style.width =
						draggingWidth + "px";
				}
				// if element the draggedItem is dragged onto is within the array of all elements in list
				// (not only items, but also disabled, etc.)
				if (
					Array.from(sortableElement.children).indexOf(element) > -1
				) {
					...
					if (
						thisHeight > draggingHeight ||
						thisWidth > draggingWidth
					) {
						// Dead zone?
			...
			var onDragOverEnter = function (e) {
				...
				var sortableElement =
					element.isSortable === true
						? element
						: findSortable(element, e);
				...
				e.dataTransfer.dropEffect =
					store(sortableElement).getConfig("copy") === true
						? "copy"
						: "move";
				debouncedDragOverEnter(
					sortableElement,
					element,
					e.pageX,
					e.pageY
