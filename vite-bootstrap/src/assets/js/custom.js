(function () {
	...
	// RTL Toggle
	const getRTLToggleId = document.getElementById('ltr-rtl');
	if (getRTLToggleId) {
		const switchtoggle = document.querySelector(".ltr-rtl");
		const savedTheme = localStorage.getItem("trezo_theme");
		if (savedTheme) {
			document.body.setAttribute("ltr-rtl-data-theme", savedTheme);
		}
		switchtoggle.addEventListener("click", function () {
			if (document.body.getAttribute("ltr-rtl-data-theme") === "rtl-enabled") {
				document.body.setAttribute("ltr-rtl-data-theme", "ltr-enabled");
				localStorage.setItem("trezo_theme", "ltr-enabled");
			} else {
				document.body.setAttribute("ltr-rtl-data-theme", "rtl-enabled");
				localStorage.setItem("trezo_theme", "rtl-enabled");
			}
		});
	}
